...
namespace Faverou
{
    public partial class frmPagoTasadores : Form
    {
        ...
        public frmPagoTasadores()
        {
            InitializeComponent();
        }

        private void frmPagoTasadores_Load(object sender, EventArgs e)
        {
            setControls();
            syncButtons();
        }
        ...
        {
            using (var form = new frmPagoTasadoresFinder())
            {
                form.ShowDialog();
                string nombreTasador = form.tasador.Trim();
                int idTasador = form.id;

                if (!string.IsNullOrEmpty(nombreTasador))
                {
                    clearForm();

                    this.nombreTasador = nombreTasador;
                    fillHeader(nombreTasador);
                    gridBinded = false;
                    fillGrid(idTasador);
        ...
        private void fillHeader(string nombre)
        {

            lblNombreTasador.Text = nombre;
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string Query = "select * ";
                Query += "from SDMREFERENCEFORM ";
                Query += "where id = " + idTasador.ToString();
        ...
        private void changeValue(bool valor, int pos) {
            ...
            if (gridBinded) {
                ...
                if (!valor)
                {
                    ...
                    selected++;
                    lblSeleccionadas.Text = "Seleccionadas: " + selected.ToString();
                }
                else
                {
                    importeFacturado -= Convert.ToDouble(dataGridView1.Rows[pos].Cells[8].Value);
        ...
        private string validatePago() {

            string mensaje = string.Empty;

            // Valido nro de recibo
            int nroRecibo;
            if (!int.TryParse(txtNumeroRecibo.Text, out nroRecibo) || nroRecibo < 1)
                mensaje += "Debe ingresar un Nro. de Recibo válido.\n";
            ...
            if(totalFacturado <= 0)
                mensaje += "El importe facturado no puedo ser cero.\n";
        ...
        private void txtNumeroRecibo_Enter(object sender, EventArgs e)
        {
            // Kick off SelectAll asyncronously so that it occurs after Click
            BeginInvoke((Action)delegate
            {
                txtNumeroRecibo.SelectAll();
            });
        }
        ...
        private void btnReimprimir_Click(object sender, EventArgs e)
        {
            ...
            using (var form = new frmPagoTasadoresPrint())
            {
                form.ShowDialog();
            }
